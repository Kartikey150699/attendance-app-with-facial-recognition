...
function FaceTracker({ selectedCamera, onDetectionsChange, facesRef }, ref) {
  ...
  const drawDetections = (detections, backendFaces) => {
    ...
    detections.forEach((d) => {
      ...
      grad.addColorStop(0, "rgba(34,197,94,0.9)");   // near = green
      grad.addColorStop(0.5, "rgba(234,179,8,0.9)"); // mid = yellow
      grad.addColorStop(1, "rgba(239,68,68,0.9)");   // far = red
      ctx.fillStyle = grad;
      ...
      if (distanceCm < MIN_DISTANCE) glowColor = "rgba(59,130,246,0.9)";   // too close = blue
      else if (distanceCm > MAX_DISTANCE) glowColor = "rgba(239,68,68,0.9)"; // too far = red
      else glowColor = "rgba(34,197,94,0.9)"; // perfect = green
